...
def phn_test(dataset,path_data,save_weights,device,mode):
    ...
    if mode == "loss":
        if dataset == 'mnist':
            ...
            plt.plot(loss_hv1[:, 0], loss_hv1[:, 1],label = 'HPN-Utility',marker='o',linestyle = '-.')
            plt.plot(loss_hv2[:, 0], loss_hv2[:, 1],label = 'HPN-Cheby',marker='x',linestyle = ':')
            plt.plot(loss_hv3[:, 0], loss_hv3[:, 1],label = 'PHN-EPO',marker='v',linestyle = '--')
            x = [0.2,0.3,0.4,0.5]
            y = [0.2,0.4,0.6,0.8]
            plt.plot([0.2631959812361983,0.2631959812361983],[0.2,0.8],ls='-.',c='black',label='Single task')
            plt.plot([0.2,0.5],[0.33708666510219815,0.33708666510219815],ls='-.',c='black')
            ...
            plt.savefig('test_multi_'+str(dataset)+'.jpg')
            plt.savefig('test_multi_'+str(dataset)+'.pdf')
        print("HV HPN-LS: ",hypervolumn(np.array(loss_hv), type='loss', ref=np.ones(2) * 2))
        print("HV HPN-Utility: ",hypervolumn(np.array(loss_hv1), type='loss', ref=np.ones(2) * 2))
        print("HV HPN-Chebyshev: ",hypervolumn(np.array(loss_hv2), type='loss', ref=np.ones(2) * 2))
        ...
        if dataset == 'mnist':
            ...
            loss_hv2 = np.array(loss_hv2)
            loss_hv3 = np.array(loss_hv3)
            plt.plot(acc_task_1_0 ,acc_task_2_0,label = 'HPN-LS',marker='*',linestyle = '-')
