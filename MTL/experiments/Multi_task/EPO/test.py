...
def eval_hv(n,dataset,save_weights,test_loader,device):
    ...
    model.model.load_state_dict(torch.load(os.path.join(save_weights,'epo_'+str(dataset)+'_lenet_niter_150_'+str(n)+'.pickle')))
...
def circle_points(K, min_angle=None, max_angle=None):
    ...
    y = np.sin(angles)
    return np.c_[x, y]
def epo_test(dataset,test_loader,save_weights,device):
    loss_hv = []
    for i in range(5):
...
if __name__ == "__main__":
    ...
    epo_test(dataset,test_loader,save_weights,device)
