...
from pareto_mtl_search import pareto_mtl_search
import torch
...
def train_2d(n=2,max_iters = 100,step_size=0.1):
    ...
    for k, r in enumerate(rs):
        ...
        print("solution: ",find_target(pf, context=r))
        ...
        #x0[range(1, n, 2)] = 0.3
        x0 = np.array([x0])
        sol, res = pareto_mtl_search(concave_fun_eval, ref_vecs=rs, x=x0,i=k, t_iter=max_iters,step_size=step_size)
        # target.append(find_target(pf, context=r))
        print(res[-1])
        predict.append(res[-1])
    end = time.time()
    ...
    ax.set_ylabel(r'$f_2$')
    ax.legend()
    plt.savefig('/home/tuantran/Documents/OPT/Multi_Gradient_Descent/HPN-CSF/MOP/PMTL/2d_ex1.jpg')
    plt.show()
...
def train_3d(n=3,max_iters = 100,step_size=0.1):
    ...
    for k, r in enumerate(rs):
        ...
        x0[range(2, n, 2)] = 0.6
        x0 = np.array([x0])
        #ref_vecs,x,i,t_iter = 1000, n_dim = 20, step_size = 1
        sol, res = pareto_mtl_search(concave_fun_eval_3d, ref_vecs=rs, x=x0,i=k, t_iter=max_iters,step_size=step_size)
        target.append(find_target(pf, context=r))
        predict.append(res[-1])
    end = time.time()
    time_training = end-start
    ...
    ax.set_ylim(0, 1)
    ax.set_zlim(0, 1)
    plt.savefig('/home/tuantran/Documents/OPT/Multi_Gradient_Descent/HPN-CSF/MOP/PMTL/3d_ex3.jpg')
    plt.show()
...
if __name__ == "__main__":
    ...
    if args.mode == "2d":
        pf = create_pf5() 
        train_2d(n=1,max_iters = 1000,step_size=1)
    else:
        pf  = create_pf_3d()
        train_3d(n=3,max_iters = 200,step_size=1)
